...
function initWindows() {
	...
	for(j=0; j < pageItemsArray.length; j++){
	 	...
	 	if (pageItemsArray[j].type == 10) {
			$("#page").append("<div class='hmi-wrap' id='wrapid"+pageItemsArray[j].id+"' onclick='LampClicked("+pageItemsArray[j].id+");' style='cursor:pointer;background:url(img/checkbox_off.png);position:absolute;left:"+pageItemsArray[j].xpos+"px;top:"+pageItemsArray[j].ypos+"px;width:14px;height:14px;'><div id='lampid"+pageItemsArray[j].id+"'><img width='14' height='14' src='img/checkbox_on.png'></img></div><div style='cursor:pointer;position:absolute;left:20px;top:-2px;width:200px'>"+pageItemsArray[j].name+"</div></div>");
			...
			$("#page").append("<div class='hmi-wrap' id='wrapid"+pageItemsArray[j].id+"' onclick='LampClicked("+pageItemsArray[j].id+");' style='cursor:pointer;background:url(img/IDPNG_Light_Off@2x.png);position:absolute;left:"+pageItemsArray[j].xpos+"px;top:"+pageItemsArray[j].ypos+"px;width:"+pageItemsArray[j].width+"px;height:"+pageItemsArray[j].height+"px;'><div class='lampclass' id='lampid"+pageItemsArray[j].id+"'><img src='img/IDPNG_Light_On@2x.png'></img></div></div>");
			for(k=0; k < devicesArray.length; k++){	
				if( devicesArray[k].id == pageItemsArray[j].device_id) {
					if(devicesArray[k].val == 0){
						//$("#lampid"+devicesArray[k].id).fadeOut();
...
jQuery(function($){
	...
	socket.on('sendallvalues', function(data){
        ...
        for(j=0; j < ValuesA.length-1; j++){
            ...
            if (OneValueA[0]==4002){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4002").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4003){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4003").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4004){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4004").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4005){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4005").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4006){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4006").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4007){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4007").html(OneValueA[1]+' ªC');}
            if (OneValueA[0]==4008){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);}
            if (OneValueA[0]==4009){$("#listTempVal"+OneValueA[0]).html(OneValueA[1]);$("#mainTempVal4009").html(OneValueA[1]+' ªC');}
			...
			if (OneValueA[0]==3001){$("#listPowerVal3001").html(OneValueA[1]);$("#mainPowerVal3001").html(OneValueA[1]+' V');}
			if (OneValueA[0]==3002){$("#listPowerVal3002").html(OneValueA[1]+' A');$("#mainPowerVal3002").html(OneValueA[1]+' A');}
			if (OneValueA[0]==3008){$("#listPowerVal3008").html(OneValueA[1]);$("#mainPowerVal3008").html(OneValueA[1]+' W');}
			
			if (OneValueA[0]==3003){$("#listPowerVal3003").html(OneValueA[1]);$("#mainPowerVal3003").html(OneValueA[1]+' Ah');}
			if (OneValueA[0]==3004){$("#listPowerVal3004").html(OneValueA[1]);$("#mainPowerVal3004").html(OneValueA[1]+' %');}
